...
pub fn run_from_args_string(arg_str: &str) -> Result<(), Error> {
    // let args = Cli::parse();
    let argv = shell_words::split(arg_str).context("parsing CLI string")?;
...
#[unsafe(no_mangle)]
pub extern "C" fn run_iso2god(args: *const c_char) -> i32 {
    let c_str = unsafe {
        assert!(!args.is_null());
        CStr::from_ptr(args)
    };
    let args_str = match c_str.to_str() {
        Ok(s) => s,
        Err(_) => return 1,
    };
    println!("{}", args_str);

    match run_from_args_string(args_str) {
        Ok(_) => 0,
        Err(e) => {
            eprintln!("Error: {:?}", e);
            1
